...
"@media(max-width:720px){.log-out{white-space:pre-wrap;overflow-x:hidden;}}"
        ...
        H("var _gpR=0;"
          ...
          "});gl.innerHTML=h;"
          "}else{"
          "gl.innerHTML=((!gd||gd.error||gd.status)?'<p class=\"hint\" style=\"color:#f87171;\">SM API nicht erreichbar</p>':'<p class=\"hint\">Keine Spiele gefunden</p>');"
          "}"
          "if(!il)return;"
          "if(!id||id.error||id.status){il.innerHTML='<p class=\"hint\" style=\"color:#f87171;\">'+(id&&id.error?id.error:'nicht erreichbar')+'</p>';return;}"
          ...
          "id.images.forEach(function(img,i){"
          ...
          "var iba=img.mounted"
          "?'<button onclick=\"_smUnmount('+i+')\" style=\"background:transparent;border:1px solid #f87171;border-radius:4px;color:#f87171;padding:2px 8px;cursor:pointer;font-size:.7rem;\">Unmount</button>'"
          ":'<button onclick=\"_smMount('+i+')\" style=\"background:transparent;border:1px solid var(--accent);border-radius:4px;color:var(--accent);padding:2px 8px;cursor:pointer;font-size:.7rem;\">Mount</button>';"
          ...
          "ih+='<td style=\"padding:6px 8px;text-align:center;\">'+st+' '+iba+'</td></tr>';"
          ...
          "function _smUnmount(i){var g=window._smG&&window._smG[i];if(g&&g.tid)unmountGame(g.tid);}");
        H("function smRawDebug(){"
          ...
          "fetch('/api/sm/games/mount?title_id='+encodeURIComponent(tid),{method:'POST'})"
          ".then(function(r){return r.json();})"
          ".then(function(d){"
          "if(d.error&&el)el.innerHTML='<p class=\"hint\" style=\"color:#f87171;\">'+d.error+'</p>';"
          "setTimeout(function(){loadGamePanel(0);},3000);})"
          ".catch(function(){setTimeout(function(){loadGamePanel(0);},3000);});}"
          "function unmountGame(tid){"
          "var el=document.getElementById('games-list');"
          "if(el)el.innerHTML='<p class=\"hint\">Unmounting '+tid+'...</p>';"
          "fetch('/api/sm/games/unmount?title_id='+encodeURIComponent(tid),{method:'POST'})"
          ".then(function(r){return r.json();})"
          ".then(function(d){"
          "if(d.error&&el)el.innerHTML='<p class=\"hint\" style=\"color:#f87171;\">'+d.error+'</p>';"
          "setTimeout(function(){loadGamePanel(0);},3000);})"
        ...
        if(!tid[0]) { mg_http_reply(c,400,"Content-Type: application/json\r\n","{\"error\":\"missing title_id\"}"); }
            ...
            char *r=sm_api_req("POST",smpath,body);
            if(!r) mg_http_reply(c,503,"Content-Type: application/json\r\n","{\"status\":503,\"error\":\"SM API not available\"}");
            else { mg_http_reply(c,200,"Content-Type: application/json\r\n","%s",r); free(r); }
        }
    }
